...
- Name: 01-public.spec.ts >> Public Endpoints >> P-04: Protected endpoint without auth returns 401
- Location: e2e\scenarios\01-public.spec.ts:34:7
...
```ts
  ...
  32 |   });
  33 | 
  34 |   test('P-04: Protected endpoint without auth returns 401', async () => {
  35 |     const request = (test as any).request || this.request;
> 36 |     const response = await (request as any).get(`${API_BASE_URL}/audit-logs`);
     |                                             ^ TypeError: request.get is not a function
  37 | 
